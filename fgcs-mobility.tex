...
Proposed device is a single-board computer, based on the Raspberri
...
\item \textit{Alejandría}: This is the system database. It is a MySQL instance placed in a local server. It is optimised (using indexes and table partitioning) to provide a close to real-time processing and data service.
...
MOBYWIT's system is tested in four different scenarios, described in the following sections, in order to prove its value in different urban problems.
...
Figure \ref{fig:nodosDGT} shows the identifiers of these devices and their 
 placement. Exact locations is described in Appendix section. These locations have been chosen because they are the nearest   
ones to existent loop detectors monitored by the DGT, and also, they have an
...
\begin{figure}[htb]
	\begin{center}
		...
		\caption{Average speen between nodes, detecting the unique vehicle identifiers by our MOBYWIT devices. Every day is grouped in blocks of 4 hours.}
