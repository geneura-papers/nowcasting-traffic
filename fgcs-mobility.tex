...
This paper presents a set of applications of mobility
monitoring system designed by us that addresses several problems in a
Smart City scope. % ¿Qué es un smart city scope? - JJ 
This system tracks the movement of people and vehicles analysing the
radioelectric space,  % no hace eso. Simplemente captura paquetes - JJ
specifically, the  WiFi and Bluetooth signals emitted by personal
(smartphones) or on-board (hands-free) devices. 
The study has been conducted in four different real scenarios,
...
traffic tracking (urban and intercity roads). %¿Study de qué? ¿Por qué
                                %quieres estudiar eso? ¿Por qué
                                %quieres hacer un estudio? - JJ
Different data mining techniques have been applied to extract useful
knowledge, traffic forecasting methods to perform reliable
predictions, and statistical analyses to model and validate the system
accuracy (using different data sources). % accuracy ¿con respecto a
                                % qué? ¿Por qué has capturado esos
                                % datos y has usado esas técnicas?
The obtained results in all the cases show the viability and utility
of the system, along with its multiple applications for solving real
Smart City issues. % ¿cómo muestran sus aplicaciones? ¿Cuales son los
                   % "smart city issues"?- JJ
