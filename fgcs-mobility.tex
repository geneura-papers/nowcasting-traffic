...
%
\begin{abstract}
This paper presents a novel mobility monitoring system and some of its applications to address problems that would be solved in a Smart City, such as the optimisation of traffic flows in terms of trip-time and security (Smart Traffic), and the improvement of security or energetic issues inside buildings.
...
This information can be directly used inside a city in several ways, from improving the traffic flow, by adjusting the synchronisation of traffic lights or defining the best traffic sense on every street, to the end-users (drivers), who can profit by optimally planning their trips according to the current, or future, status of the traffic flow. 
That's the reason why traffic information must be reliable. 

This paper presents a monitoring system able to collect people's and vehicles mobility data, and also able to track them by means of a grid of devices, or nodes, connected to a central server. This allows the easy construction of traffic flow models or mobility graphs. The proposed system is formed by low-cost devices, around 100\$ each, which is another advantage over the usual monitoring systems, because the cost is an important factor for authorities which
tend to install the simplest and cheapest monitors. 

The device, a highly improved version of the one presented in
\cite{castillo2014_book_sipesca}, is based on a single-board computer
which monitors the radioelectric space, capturing Bluetooth (BT) and
WiFi signals emitted by other devices,  mainly
smartphones or hands-free systems on vehicles. These devices are detected and
...
% significa - JJ
techniques, transforming or summarising them, in order to conduct
...
\textit{non-intrusive}, without contact with the road, causing a minimal
effect on the traffic flow \cite{martin2003detector}. 

As stated in the previous section,the main technologies currently used include pneumatic tubes, inductive loop detectors and automatic recognition systems (or OCR), among others \cite{rodrigue2013geography}.
...
not be the best, as the weather may limit accuracy. Moreover, these systems are 
...
management system based on BT to count people and commercial routes 
(pathsolver). Its technology was deployed in highways managed by Abertis for 
traffic flow control and monitoring. Currently, it has a network of 150 devices 
...
The forecasting of these flows in any transport system plays an important role for either alleviating urban congestion, or for adjusting traffic control and guidance systems. The aim is to create an intelligent transportation system (ITS) which strongly depends on the capability to predict traffic conditions, at least in the short-term (less than one hour into the future), since medium- and long-term predictions are barely accurate and reliable. These are nowadays a hard problem to solve in general Time Series \cite{ParrasGutierrez2014433}.
...
There are many works related to traffic flow forecasting, each of them approaching the problem with different models, for instance: Kalman Filter Theory \cite{ASJC:ASJC1051}, Markov Chain models \cite{Qi201495}, non-parametric methods \cite{Noparametrico2014}, simulation models \cite{simulaciontrafico2012} or Big-Data-based approaches \cite{SurveyKumar2007}.  
%
Moreover, being the traffic flow forecasting a trending topic, several surveys can be found in the literature, such as \cite{Survey2004} and \cite{SurveyKumar2007}; but the best is the one by Bolshinsky and Friedman \cite{SurveyBolshinsky2012}, since  the authors complete the information that the previous ones just compile, and give an added value to their work including the used techniques, the input data for each contribution, and the used hardware architecture, whenever the original paper described it. There are also specific surveys, like \cite{Vlahogianni2014}, which presents a classification of the revised papers regarding the main contributions of the work, i.e. forecasting techniques, gathering systems, or datasets, among others.


In the last years the researches even assimilate new technologies
for data analysis in this scope, such as Big Data \cite{TrafficFlowPredictionWithBigData2015} or Cloud-Computing approaches \cite{CloudComputing2014}. In the first one, Yisheng et al. face the traffic flow forecasting problem using temporal and spatial models at the same time, using simple forecasting models and assembling them in a global complex system. The results provide a time and a spatial place for each prediction. The second work, by Zhaosheng Yang et al., presents a traffic flow prediction model for large-scale road network using Cloud-Computing techniques. Thus, it implements
successfully a Genetic Algorithm (GA) for optimising a Support Vector
Machine (SVM) getting a higher prediction accuracy and a smaller
running time.  

Several recent publications present spatial issues related to the
...
Another line of study in our paper is the personal tracking or people's mobility. It is studied for many reasons, several of them involving profit: conversion rate, marketing effectiveness or staff planning are some of the best known \cite{stanton1967fundamentals}. Conversion rate is the percentage of visitors who effectively make a purchase. It can show you if a shop improvement (or decline) is due to more people visit it, or more of the visitors finally buy something. Shopping mall professionals study people's behaviour in order to know if their marketing techniques are paying off. In the same line, staff planning is directly related with the amount of visitors, as well as the maintenance and cleaning needs.

Other reasons are more practical than commercial: How many people visit a public building? When? It will help to improve the planning of the provided services, heating or cooling of the building, facility cleaning, staff shifts and so on. In a museum or a sports stadium you can adjust many parameters to help people enjoy from a better experience while raising the profits.

The technology for counting/tracking people has evolved over time. At the beginning infrared beam counters \cite{Hashimoto199850} were used. These were cheap but prone to failures, like when two persons pass a door together. Thermal counters \cite{635472} were used next. Also cheap, but with serious limitations like the short distance from the counting device to the people being counted. Latest technology uses video cameras \cite{4107051,Schofield19961421} and WiFi \cite{7134066,7130402}, or Bluetooth \cite{6526732,nicolai2007relationship} detectors. 
...
Also the idea of tracking people with their personal WiFi devices has been exploited. As more and more people use it every day it is becoming one of the best methods to follow them. This idea has been put into practise recently by several authors, for instance: Wei Xi in \cite{6847958} demonstrates how this method can count people with great accuracy, and Gabe Fierro in \cite{Fierro:2012:ZOC:2422531.2422572} uses it to measure the occupancy level of a building without requiring additional infrastructure.

However, by joining BT and WiFi we pursue to increase the detection rate further than most people has achieved. Apparently it can seem redundant, but the detection likelihood is higher. We need to study more deeply the improvement of the combined detections but on average we detect a 80\% of WiFi devices and a 20\% of BT. BT is specially important for the detection of cars because it is a technology included in almost every new car. The same fact is true for WiFi in  smartphones. Even when people disconnect WiFi devices keep on sending signals.
...
MOBYWIT system is tested in four different scenarios, described in the following sections, in order to prove its value in different urban problems.
...
Firstly, we were interested in checking the possibilities of the system in an environment where change is very fast and it is, a priory, possible to detect a big amount of devices. Thus we decided to test MOBYWIT in a disco, using it also as a test for its utility inside buildings.
Five devices have been deployed, one in every of the three main
rooms, each one devoted to a different type of music, one on the main
entrance, and one on the outdoor terrace (see the Appendix for more details). This scenario is interesting because it is assumed that 100\% of users have a smartphone, thus it would be a priori possible to track every single present device. The electromagnetic scenario is also quite {\em heavy}, which presented a challenge for testing the device.

After processing and summarising data, a total of 2200 different devices were detected in one of the busiest nights in September (from a Thursday at 8 pm to Friday at 8 am), following the distribution per node shown in Figure \ref{fig:disco_data}. 

\begin{figure}[ht]
	\begin{center}
		%%%\includegraphics[width=\textwidth]{imgs/DISCO/time_series-linesPoints.eps}
		\includegraphics[width=\textwidth]{imgs/DISCO/time_series-bars.png}
		\caption{Detected devices distribution in a night in the Discotheque scenario by room.}
		\label{fig:disco_data}
	\end{center}
\end{figure}

These data have been processed extracting several variables from them, in order to apply clustering methods. The aim is to find group behavioural patterns in the data, which could aid to make decisions about marketing or security in the building, for instance.

The clustering study has been conducted by means of a Self-Organising
...
uses SOM yielded neurons' code-vectors (vectors of variables of the
...
The features have been extracted from the gathered `events', composing a pattern per device of its whole stay in the disco (for one night). These are described in Table \ref{tab:extracted_features_disco}.
...
\noalign{\smallskip}\hline
\end{tabular}
 \caption{Extracted features from gathered data in the discotheque for one night. \label{tab:extracted_features_disco}}
}
\end{table}
...
\begin{figure}[!ht]
	\begin{center}
		...
		\caption{Planes Analysis (top) and U-MATRIX Graph (bottom) obtained from applying SOM to the dataset of detected devices in a night in the Discotheque scenario. The planes show a projection of the values of the extracted features described in Table \ref{tab:extracted_features_disco}}
		\label{fig:som_disco_sin_no_detec}
	\end{center}
\end{figure}

Several clusters (blue areas surrounded by other colours) arise in U-MATRIX graph. Looking at them, it can be seen that the central cluster is strongly related to high values of features of node 102 (see SOM Planes Analysis on the top of the figure): `abs\_time\_n102', `rel\_time\_n102',  and `rel\_time\_nig\_n102' which is on the Terrace. Thus, having in mind the dates and the good temperature, they are probably people staying there, not just going out for smoking as would be usual in other seasons.

Top-right cluster corresponds to people with an early entrance and out (`ent\_time', `out\_time') and little stay time ('stay\_time') as plotted in the Plane Analysis. This could correspond to people just staying for a beer or coffee, before the `hard music' starts.

The absence of labels in the U-MATRIX graph limits the possible interpretation and conclusions that we can reach from it. SOM Planes Analysis is more useful this time.

They display, for instance, that relative time with regard to the whole opening time of the disco shows obvious information, such as that this value is small for node 132, placed in the ticket office. But also, useful knowledge, like that room of node 112, the Main Room, is the most successful (more devices remain there during more time), followed by the Minor room 1 (node 122).

Another interesting result is that people (their devices) tend to be just in one or two rooms, that's why there are several 0 values in the time features per node. 

This study has been complemented with another one using again SOM and U-MATRIX methods. This time we want to analyse the \textit{recurrence of devices} to the discotheque, and their overall behaviour in the days they have come back. To this aim a new dataset has been composed, extracting those `events' of devices detected in more than two nights in two whole months.
A similar set of features has been extracted for this experiment, as they are shown in Table \ref{tab:features_recurrence_disco}.
...
\noalign{\smallskip}\hline
\end{tabular}
 \caption{Extracted features from gathered data in the discotheque for returning devices. \label{tab:features_recurrence_disco}}
}
\end{table}

For each device, we have computed a sum of the amounts of diary time per monitoring node. Relative times have been also calculated as another set of features. There are around 220000 monitored `events' and 2100 different devices, so this is the amount of patterns to cluster.
However, this time the data have been labelled using the number of different days every device has been detected (the recurrence rate).

The results of this study are shown in Figure \ref{fig:som_disco_recurrencia}.

\begin{figure}[!htp]
	\begin{center}
		\includegraphics[width=11cm]{imgs/DISCO/som_recurrentes.eps}
		\includegraphics[width=6cm]{imgs/DISCO/umatrix_recurrentes.eps}
		\caption{SOM Planes (top) and U-MATRIX (bottom) application on the recurrence of devices in different days. The label is the recurrence rate (number of days the device has returned) of the majority in that neuron/BMU}
		\label{fig:som_disco_recurrencia}
	\end{center}
\end{figure}

As it can be seen in the U-MATRIX, every neuron (cell in the graph) has been labelled according to the class of the majority of the most similar input patterns. To this end the distance from every pattern to every neuron is computed considering their respective code-vectors, the closest neuron to an input is named the best matching unit (BMU), and it is labelled with the pattern class (stage in this problem). Once the process has finished every neuron with more than one associated class finally displays the label with more occurrences.

U-MATRIX graph shows three big clusters (blue areas), corresponding the bottom one to very frequent devices (they have returned more than 30 times). This leads us to think in staff going to work. Most of them are related to high stay times in the Terrace (node 102), as it is displayed in the Plane Analysis.
The top cluster represents the amount of recurrences of people just going to the Ticket Office, since it corresponds to an area of high values for the variable `rel\_time\_n132', but apparently, most of these people seem cannot access the rooms, as they spend a high percentage of their time close to that node.

Moreover, in the Plane Analysis, relative times in rooms of nodes 122 and 142 (Minor 1 and 2 respectively) seem to be complementary, meaning that recurrent people `strictly' choose between one room or another (depending on their musical taste).

The knowledge extracted in these two analyses could be of interest for many issues, apart from the direct testing of the occupation with regard to the disco legal capacity. 
Knowing the behaviour of people could help to improve the security in the building, making the entrance and out easier in the most populated rooms.
This system would be very useful also to analyse specific events and their development according to the behaviour of people.
MOBYWIT also provides real data about the success of any of the rooms, giving data per hour that could be of interest for improving the energy consumption (one room could open or close earlier depending on the rush hours), for instance.
The location of a node in the Terrace could be also interesting to study the habits of smokers during the cold seasons, when there is no music outside.
...
The devices have been recording data for a few days only. With these data the origin-destination histogram that can be seen in Figure \ref{fig:etsiit_io} has been created. A careful examination of this figure lets us extract some conclusions.
...
\item Why people getting in by the main door (62) almost never go out by the same door? Most of them are students and go out by their door (52) at the end of the classes.
...
With some additional sensors the people flow could be studied in a better way. Because with the three used many people can not be correctly interpreted as the right kind of visitor. The most interesting place for a new sensor is at the door of the classroom building at the north on the map.

The information gathered can be useful for the management of the building. When must heating be started? How many students are in class or just studying inside? Are there enough seats in the library? Are the doors in the right place? Many of these questions need a little more time to save enough data and answer them correctly.
...
different number of MOBYWIT devices placed in national highways.
...
have passed a point, with the number of devices detected by MOBYWIT. Thus, a 
realistic \textit{ratio} between both numbers has been computed.

Six nodes were placed along different roads in Andalusia (southern Spain). 
Figure \ref{fig:nodosDGT} shows the identifiers of these devices and their 
 placement. Exact locations are described in Appendix section. These locations have been chosen because they are the nearest   
ones to existing loop detectors monitored by the DGT, and also, they have an
...
(1010, 1020 and 1030) and the other on the road to M{\'a}laga (1040, 1050 and 1060).

\begin{figure}[htb]
	\begin{center}
		\includegraphics[scale=0.4]{imgs/nodos_dgt.eps}
		\caption{Location of MOBYWIT devices in the highway traffic scenario}
	\label{fig:nodosDGT}
	\end{center}
\end{figure}


In this section, several experiments will be conducted. First, a study on the velocity and driving behaviour taking into account the tracking capabilities of MOBYWIT will be presented. Also, different methods to extract a ratio between our gathered data and the real data provided by the DGT will be performed. Finally, a time series forecasting study will be performed. In order to compare the performance of the different methods, 4 frequently used error measures will be used in this section:

Mean Absolute Error ({\em MAE}),
Mean Absolute Percentage Error ({\em MAPE(\%)}),
Mean Squared Error ({\em MSE}), and
...
Using the time when each repetition has been detected, we can also calculate average speed velocity during the week. Figure \ref{fig:dgtMatrixTime} shows the detected times between nodes and Figure \ref{fig:dgtMatrixSpeed} the average speed, in blocks of 4 hours, during the week. Largest travel times are detected between more separated nodes. As in previous figure, time between more separated nodes is not tracked because shorter paths between the important cities exist. %***HABLAR MAS DE ESTOS DATOS***
...
\begin{figure}[htb]
	\begin{center}
		\includegraphics[scale=0.35]{imgs/PETRA/Speed/petra_graph_speed.eps}
		\caption{Average speed between nodes, detecting the unique vehicle identifiers by our MOBYWIT devices. Every day is grouped in blocks of 4 hours.}
	\label{fig:dgtMatrixSpeed}
	\end{center}
\end{figure}
...
The information MOBYWIT is able to provide for urban scope is wide, but we are mainly interested in two goals: to measure the trip time between several points in a city and the path the drivers follow across the city to get to the closest highway. Both goals are the main area of interest of the local city council. However, in this work we have focused on the first issue as a test for our system.

Traffic jams are present in every town, produced when the street/road capacity is overflowed, due to usual (rush hours) or punctual reasons (accidents). Some authors have classified the traffic jam causes \cite{CausasAtascos1,CausasAtascos2}, but they have been studied in particular cities. 

%When a driver finds himself in a traffic jam, the natural response is to plan another path to arrive his destination and the information, the competent authority provides, about other available path is appreciated by them. The selection of another path depends on driver destination and it could be different for each case. So, the first stage is to inform about the traffic jam and then the drivers decide to go across or miss it. The second stage is to provide the available paths and to turn off the vehicles, but this goal has not been addressed in the paper. 

To illustrate MOBYWIT's utility for early detection of traffic jams a small experiment has been carried out in Granada city (Spain). The goal of this experiment is to detect how the trip time varies between two nodes, A and B, in one of the main streets of the town, which is one-way, has four lanes, and it is regularly collapsed several hours a day. Figure \ref{fig:mososMapa} shows a map of this street and the location of the nodes (the exact location is shown in the Appendix). 
The distance between both nodes is $168$ metres. 
...
Firstly we should note that, obviously, our devices cannot detect all the real people and vehicles moving close to them, since not all of them emit BT or WiFi signals. However, we conducted an open poll to $600$ people from which we conclude that around the $22\%$ of the drivers have the BT of the car activated and around the $70\%$ of the drivers emit WiFi signal. We consider the sample is good and representative of the real world, so we will use these results as a reference. 

In the experiment we measure the average time taken to go from node A to B for each monitored device. The results of this experiment are displayed in Figure \ref{fig:mososTiempo}. 

\begin{figure}[!htb]
	\begin{center}
		\includegraphics[scale=0.35]{imgs/MOSOS/mosos_graph_tiempos.eps}
		\caption{Average time in a day for displacements from node A to node B, grouping the hours of the day in eight terms (lapses). The first and the eighth terms belongs to the night. The second one includes the early rush hour in the morning. The third period (from 9:00 am to 11:59 am) is normally quite. The fourth term includes a second rush time period. Fifth period is also quite, while the sixth one (from 6:00 pm to 8:59 pm) is in another rush. The boxplot represents the relative time with regard to the global average trip duration ($0.5$).}
	\label{fig:mososTiempo}
	\end{center}
\end{figure}

In this figure the day has been sliced into eight different terms (or lapses). We calculate the average trip time, using all the collected data, being this value $85$ seconds, and called Global Trip Time Average (GTTA). After that, we calculate the boxplot for each term, scale the results to $[0,1]$ interval and represent it using their rate with respect to GTTA value placed on the Y axis on $0.50$. That is, if the box is over $0.50$ means that all the trips of that term are longer that GTTA, and if the box is below $0.50$ means that all the trips of this term are shorter than GTTA. 

Figure shows that four terms are over GTTA, meaning slower periods, with terms 2, 4 and 6 above all. This is an expected result since these periods include the rush hours. However, this has been contrasted just using data provided by MOBYWIT, which means that the system can be reliable.
Thus, this could be useful to, for instance, adjust the duration of light traffic red/green intervals automatically, using these values as a reference.

%without any additional data it is easy to know if the trip times are increasing or decreasing along the day. The utility that any council is able to give to this information could be rich and diverse. For example, if any council wants to monitor the streets and links the MOBYWIT output to the traffic light network, the traffic light regulation could be autonomous depending on the time that a monitored device spends to go from one checkpoint to another. 
