...
Table \ref{tab:ratiosDGT} compares the obtained ratios, using several error measures (previously explained). As it can be seen, the ratio real/detected is around 20 vehicles for the Total, Mean and Median. However, best results in MAE are obtained when using the Median by Quarter ratio.
...
\begin{figure}[htb]
	\begin{center}
		\includegraphics[scale=0.3]{imgs/petra_graph_DGT-Mobywit-mended.eps}
...
\begin{figure}[!ht]
	\begin{center}
		...
		\caption{Real DGT vs forecasted number of cars in node 1011, according to MOBYWIT's adjusted records.}
		\label{fig:forecasted-values-fixed}
	\end{center}
\end{figure}
