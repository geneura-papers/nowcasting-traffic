...
Being the traffic flow forecasting a trending topic several surveys exist and they provide a comprehensive idea about the scope 
...
Other reasons are more practical than commercial. How many people visit a public building? When? It will help to improve the planning of the services provided, heating or cooling of the building, facility cleaning, staff shifts and so on. In a museum or a sport stadium you can adjust many parameters to help people enjoy from a better experience while raising the profit.
...
Different metrics will be used to obtain a correct correlation between the number of detected devices and the real number detected by the ESPIRAS.

\begin{itemize}
\item Total ratio: ratio between the number of vehicles detected by the ESPIRAS and the number of vehicles detected by our device.
\item Average of ratios: as the maximum granularity of the data provided by the DGT is 15 minutes, the sum of all ratios of each 15 minutes section has been divided by the total number of sectinons.
\item Median ratio: instead the average ratio as in previous metric, this one uses the median.
\item Time ratio: se calcula el ratio pero agrupado por las mismas horas del (por ejemplo: vehiculos dgt/vehiculos nuestros a las 12:00)
\item Igual con mediana?
\end{itemize}
