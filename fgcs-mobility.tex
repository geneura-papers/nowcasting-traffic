...
There are many works related with traffic flow forecasting, each of them approaching the problem with different models, for instance: Kalman Filter Theory \cite{ASJC:ASJC1051}, Markov Chain models \cite{Qi201495}, non-parametric methods \cite{Noparametrico2014}, simulation models \cite{simulaciontrafico2012} or Big-Data-based approaches \cite{SurveyKumar2007}.  
%
...
Also the idea of tracking people with their personal WiFi devices has been exploited. As more and more people use it every day it is becoming one of the best methods to follow them. This idea has been put in practise recently by several authors, for instance: Wei Xi in \cite{6847958} demonstrates how this method can count people with great accuracy, and Gabe Fierro in \cite{Fierro:2012:ZOC:2422531.2422572} uses it to measure the occupancy level of a building without requiring additional infrastructure.

However, by joining BT and WiFi we pursue to increase the detection rate further than most people has achieved. Apparently it can seem redundant but the detection likelihood is higher. We need to study more deeply the improvement of the combined detections but on average we detect a 80\% of WiFi devices and a 20\% of BT. BT is specially important for the detection of cars because it is technology included in almost every new car. The same fact is true for WiFi in  smartphones. Even when people disconnect WiFi devices keep on sending signals.
...
\item \textit{Raziel}: This module is in charge of the detection of BT and WiFi devices as well as of their identification (extracting and encrypting the MAC) and the periodic submission of this information to the server. It runs in the device and has been implemented in Java.
...
\item \textit{Ezequiel}: This module is responsible of the publication of data in a cloud-based storage. It includes data mining, machine learning and forecast techniques to process these data in order to publish interesting or useful information about them.

\item \textit{Anan}: This is the cloud-based storage and services. It is based in Google technology so Anan stores and manages the data in a NoSQL format, by means of Google Fusion Tables. It also offers advanced visualisation methods to be more usable and attractive for the end-user of the system.
...
MOBYWIT's system is tested in four different scenarios, described in the nest sections, in order to prove its value in different urban problems.
...
Table \ref{tab:ratiosDGT} shows the obtained ratios. Results show that our device detects one vehicle per approximately 19 detected by the DGT, according Total, Mean and Median ratios. However, best results in MAE and MAPE are attained obtaining ratios dividing by quarter.
...
\caption{Error values computed using the differences between DGT real data and data recorded by MOBYWIT with Median Ratio By Hour conversion. Period to be forecasted started on 25-Oct-2015, 00:00 hours and finished on 31-Oct-2105, 23:45 hours, establishing the horizon to 1. Best (=lowest) values are highlighted in bold.}
