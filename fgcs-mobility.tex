...
the system would be (potentially) suitable to be part of a \textit{Smart City}. 
...
There are many works related with traffic flow forecasting, each of them approaching the problem with different models. These include: series models, Kalman Filter Theory \cite{ASJC:ASJC1051}, Markov Chain models \cite{Qi201495}, non-parametric methods \cite{Noparametrico2014}, simulation models, local regression models, fuzzy-neural approaches, sequential learning, spectral analysis, Bayesian networks, neural networks, diffusion models or Big Data based approaches \cite{SurveyKumar2007}.  %(buscar referencias recientes de cada una y ponerlas si queremos ampliar este párrafo)
% Antonio - pos eso. ;D
%
...
In order to analyze the data, several features have been extracted (generated) from the gathered `events', composing a pattern per device of its whole stay in the disco. These are described in Table \ref{tab:features_disco}.
...
To illustrate the Mobywit utility for the first stage, early detection of a traffic jam is essential, and a little experiment has been carried out in Granada city (Spain). We have installed two Mobywit devices in the same street. The device's location is Dr. Ol\'{o}riz street. It is one-way, has four lanes and is placed in a neighbourhood which regularly is collapsed. The map of the devices location is in Figure \ref{fig:mososMapa}. The distance among both nodes is about $168$ metres. These devices cannot detect all the real persons and vehicles moving close to them since not all of them emit BT or WiFi signals, however after an open poll which $600$ people answered we conclude that around the $22\%$ of the drivers emit BT and around the $70\%$ of the drivers emit WiFi signal. We consider the figure is good and representative of the real traffic flows. For this starting experiment, we measure the taken time to go from the first to the second Mobywit device for each counted people. The results of this experiment are displayed in Figure \ref{fig:mososTiempo}. In this figure, we have defined six terms for the twenty-four hours of one day. We calculate the average of trip time using all the data and the result is $85$ seconds. We referee this value as global trip time average. After that, we calculate the boxplot for each term and represent it using their rate respect to the global trip time average. That is, if the box is over $0.50$ means that all the trips of that term are longer that $85$ seconds and if the box is below $0.50$ means that all the trips of this term are shorter than the global trip time average. As you can see in the Figure, first, second and third terms are over $0.50$ so all the trips of these terms are longer than the global average which indicates the trips during these terms are slower, increasing the average trip time until $70\%$ in the third term, that is, the trips spent $20\%$ more time.
...
\begin{figure}[htb]
	\begin{center}
		...
		\caption{Average time along the day for movements from node A to node B. We group the hours of a day using five terms. The first and sixth terms include the night, the second one includes the rush hour for morning beginning. The third period includes from 11:00 to 14:59, term including the rush hour for jobs, shools, etc. check out. The fourth term include the evening until 18:59. And the last term includes late of the afternoon until 22:59. In the boxplot se can see the percentage of increment (up to 0.50) or decrement (low to 0.50) of the time needed to go from A to B along the day.}
