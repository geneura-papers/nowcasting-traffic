...
The information Mobywit device is able to provide for urban scope is amazing but we are mainly interesting in two goals: to mesure the trip time among several points in a city and the path the drivers follow across the city to arrive the closer highway. Both goals are the interest area of the local city council, so we start to monitoring the city for achieving them. 

The traffic jam are usually in every town and it could be produced by diverse reasons. There are several papers which clasify the traffic jam causes, \cite{CausasAtascos1}\cite{CausasAtascos2}, but they have been studied in a particular city. Therefore we can not apply the solutions provided by the authors of these papers to any city, but we should solve the problem in a particular way for each one taken the previous studies as starting point for our case and getting the good ideas for us. The traffic jam are produced when the way or the street capacity is overflow for any reason during some time. Whatever the cause of the overflow, a unusual reason as a traffic accident or a cause that remains over time as vehicle increasing around a new city area, drivers should avoid the area of ??the jam.
