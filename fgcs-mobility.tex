...
This paper presents the possible applications of an own mobility monitoring system to solve problems in a Smart City. 
This system tracks people and vehicles movement using radio-electric space, specifically, the  WiFi and Bluetooth signals emitted by near personal devices, such as mobile phones and hands-free tools. %It also stores an unique, but anonymous, identifier to allow the tracking.
...
Different Data-Mining techniques have been applied to extract knowledge, traffic forecasting methods to perform reliable predictions, and statistical analyses using different data sources to validate the system accuracy.

The obtained results shows the viability ¿? of the system and its multiple applications for real Smart Cities issues. 
...
\begin{figure}[!ht]
	\begin{center}
		\includegraphics[height=4cm]{imgs/etsiit.png}
		\caption{ETSIIT MOBYWIT device location}
		\label{fig:etsiit}
	\end{center}
\end{figure}

Because devices can capture data from the university staff and the students two of them has been placed very close of the main access trying to distinguish staff (62) from students (52). The parking door (42) can be used only by the staff.
...
ORDEN
Matriz de E/S con frecuencia
Matriz de E/S con tiempos
Grafica de Velocidades


\begin{figure}[htb]
	\begin{center}
		\includegraphics[scale=0.4]{imgs/PETRA/matrix/petra_matrix_freq.eps}
...
\begin{figure}[htb]
	\begin{center}
		\includegraphics[scale=0.4]{imgs/PETRA/matrix/petra_matrix_travelDuration.eps}
...
\subsection{Data validation using other sources}
