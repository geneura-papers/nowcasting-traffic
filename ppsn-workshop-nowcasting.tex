...
% (Paloma) En realidad las espiras tampoco son intrusivas para los coches, así que no es algo que podamos destacar de mobywit. Con lo de perder vehículos se podría decir "less accurate" por ejemplo. Se pierden simplemente porque para detectarlos usamos una característica (BT) que no está en todos los coches :( (por ahora)
...
% (Paloma) He redactado again para incluír lo de que extrae las MACs, que son únicas, y para que quede claro que el tráfico que medimos es el de los coches, por eso te sonaba el cambiar generated por associated (que en realidad no es correcto por las collocations).
...
% (Paloma) Es verdad, pero nos estamos centrando en tráfico, no estamos vendiendo un chacharrico que puede ubicar personas también. Lo de las personas es un extra que no se tiene por qué mencionar.
...
% (Paloma) ¿Por qué? incidentally es como decir "by the way", que no queda mal en la frase.
...
, that is, predict 
% Antonio - yo añadiría ', rather than just detect,' o ', in addition to detect,'
...
% Antonio - esta sección habría que ampliarla un poco, ¿no? Sólo hay dos referencias a trabajos similares y no se ahonda siquiera en ellos.
% (Paloma) Agreed.
...
Bluetooth (BT) and WiFi antennas connected via USB and configured in monitor mode (WiFi) and discovering mode (BT), so that they can scan the radioelectric space searching for BT devices and beacon frames (for WiFi).
...
In order to check it, the Spanish national traffic agency (DGT) made available data from their own spirals, situated near or in the same place as our MOBYWIT nodes. This data are available from the same repository as this paper,
\url{https://github.com/geneura-papers/nowcasting-traffic}. 
% Antonio - entonces en el paper decimos de dónde se pueden descargar los fuentes del paper??? Yo pondría el enlace a los datos: https://github.com/geneura-papers/nowcasting-traffic/tree/master/datasets/PETRA
These data
are from a single month in a single year, and only during 3 hours per day,
% Antonio - supongo que eran 3 horas por día
although we have different days the week. The data is not too comprehensive, but at least gives us an idea of how the existing factors might influence the outcome. 

% Antonio - en algún sitio habría que describir el escenario donde se recopilan los datos, ¿no? O al menos decir que es una autovía entre Granada y Málaga (o donde esté el punto que se ha considerado).

One of the things we will have to take into account is that our device
only detects WiFi and Bluetooth devices. In the case of vehicles, they
are mainly hands-free Bluetooth devices. These devices have to be
...
shown. 
% Antonio - ¿Cómo se sabe qué nodo es ese? Poner una imagen o algo decir dónde está...
...
% Antonio - Explicar qué implica o quiere decir este último apunte
...
% Antonio - no se ve apenas el título lateral de las gráficas. Yo lo quitaría y lo diría en el pie de la figura
	\label{fig:dgtRatios}
	\end{center}
\end{figure}
% Antonio - se podría mostrar también la gráfica de MOBYWIT sin ajustar, para ver el efecto del ajuste. ;)
...
% Antonio - yo pondría una cita a cada uno
...
\begin{figure}[htb]
	...
	\end{center}
\end{figure}
% Antonio - no se ve la cabecera de las figuras. Quitar texto y poner sólo el título 

As is usually the case, 
% Antonio - esto lo quitaría
...
% Antonio - ¿Qué es el 'conditional algorithm'?
...
of the week. 
% Antonio - estos valores no sé si son indicativos de algo, me refiero a los valores en sí o su magnitud, para un lector no experto.
...
% Antonio - Esta explicación es un tanto 'vaga'. Si se ponen los valores, yo daria una explicación basada en ellos.
...
ratio of detected to 
% Antonio - to -> versus
actual might be different for different
...
with 400 trees.
% Antonio - yo habría incluido una breve explicación del R y qué indica su valor par alos lectores no expertos. ;)
Besides, in this last case the hour matches its
importance to the detected values, 
% Antonio - ¿dónde puede ver el lector lo que se cuenta de la hora?
...
% Antonio - ¿todas estas conclusiones dónde puede verlas el lector? No hay datos/gráficas relativas a las diferentes variables, ¿no?
...
% Antonio - alguien me dijo que las conclusiones deben ser autocontenidas. De modo que hay que decir qués es MOBYWIT, en una frase, al menos
...
vehicles as well as the day of the week and hour of these detections has proved to obtain reliably good results, although it can be overcome by random forest if
...
% Antonio - en el artículo no se discute por qué no se detectan muchos vehículos, quizá habría que ponerlo en algún punto.
